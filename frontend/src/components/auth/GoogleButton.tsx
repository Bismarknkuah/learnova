...
/** Renders the official Google Identity button when NEXT_PUBLIC_GOOGLE_CLIENT_ID is set.
 *  Without a client ID it shows a styled button that explains the one-time config step. */
export function GoogleButton({ onCredential }: { onCredential: (credential: string) => void }) {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      if (ref.current) g?.accounts.id.renderButton(ref.current, { theme: 'outline', size: 'large', width: 320, text: 'continue_with' });
    };
  ...
  if (!clientId) {
    return (
      <button onClick={() => alert('Google sign-in needs NEXT_PUBLIC_GOOGLE_CLIENT_ID (frontend) and GOOGLE_CLIENT_ID (backend). Add them to enable one-tap Google login.')}
        className="flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-ink transition hover:border-brand">
